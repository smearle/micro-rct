...
def make_RCT_env(rank: int, seed: int = 0) -> Callable:
    ...
    """
    def _init() -> gym.Env:
        env = RCT(settings_path='~/DeepLearning/micro-rct/configs/settings.yml')
        env.seed(seed + rank)
...
if __name__ == '__main__':
    ...
    rides2 = A2C(MlpPolicy, env, learning_rate = 1e-5, tensorboard_log=tb_logs, verbose=1)

    callback = CheckpointCallback(save_freq = int(1e6), save_path = model_dir)

    try:
      rides2.learn(total_timesteps=int(1e8), callback = callback)
      rides2.save("./rides2")
