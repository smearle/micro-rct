...
if __name__ == '__main__':
    ...
    model1 = A2C(ActorCriticCnnPolicy, env, learning_rate = 7e-4, tensorboard_log=tb_logs, verbose=1)

    callback = CheckpointCallback(save_freq = int(1e6), save_path = model_dir)

    try:
      model1.learn(total_timesteps=int(1e8), callback = callback)
      model1.save("./tmp/LR1model")
    except KeyboardInterrupt:
      model1.save("./tmp/test")
