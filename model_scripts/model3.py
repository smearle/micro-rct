...
if __name__ == '__main__':
    ...
    model3 = A2C(ActorCriticCnnPolicy, env, learning_rate = 1e-5, tensorboard_log=tb_logs, verbose=1)

    callback = CheckpointCallback(save_freq = int(1e6), save_path = model_dir)

    try:
      model3.learn(total_timesteps=int(1e8), callback = callback)
      model3.save("./tmp/LR1model")
    except KeyboardInterrupt:
      model3.save("./tmp/test")
