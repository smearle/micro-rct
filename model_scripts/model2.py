...
if __name__ == '__main__':
    ...
    model2 = A2C(ActorCriticCnnPolicy, env, learning_rate = 1e-4, tensorboard_log=tb_logs, verbose=1)

    callback = CheckpointCallback(save_freq = int(1e6), save_path = model_dir)

    try:
      model2.learn(total_timesteps=int(1e8), callback = callback)
      model2.save("./tmp/LR1model")
    except KeyboardInterrupt:
      model2.save("./tmp/test")
