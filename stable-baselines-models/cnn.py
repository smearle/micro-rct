...
if __name__ == '__main__':
    ...
    model6 = A2C(ActorCriticCnnPolicy, env, learning_rate = 1e-5, tensorboard_log=tb_logs, verbose=1)

    callback = CheckpointCallback(save_freq = int(4.2e5), save_path = model_dir)

    try:
      model6.learn(total_timesteps=int(1e8), callback = callback)
      model6.save("./tmp/LR1model")
    except KeyboardInterrupt:
      model6.save("./tmp/test")
